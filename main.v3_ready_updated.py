    ...
    if not await tg.is_user_authorized():
        ...
        try:
            ...
            await message.answer(
                f"❌ Не удалось отправить код Telegram:\n<code>{e}</code>"
            )
            return
        ...
        await message.answer(
            ...
            "Отправь код сюда.\n\n"
            "Можно написать его с нумерацией, например:\n"
            "<code>1.2 2.7 3.4 4.9 5.1</code>"
        )
        return
...
async def login_code(message: Message, state: FSMContext):
    ...
    # 1. 2 2. 7 3. 4 4. 9 5. 1
    # 2 7 4 9 1
    # Из первой формы получится: 27491.
    code = re.sub(r"(?<!\d)\d+\.\s*", "", raw_code)
    code = re.sub(r"\s+", "", code)

    if not code:
        await message.answer(
            "❌ Не удалось распознать код.\n\n"
            "Пример: <code>1.2 2.7 3.4 4.9 5.1</code>"
        )
        return
    ...
    try:
        ...
        await state.clear()
        await message.answer(
            "✅ <b>Telegram-аккаунт авторизован.</b>",
            reply_markup=menu(),
        )
        ...
        await state.clear()
        await message.answer(
            "❌ Код истёк. Отправь /start, чтобы запросить новый код."
        )
...
async def status_handler(callback: CallbackQuery):
    ...
    text = (
        ...
        if ok
        else "🔴 <b>Telegram не авторизован.</b>"
    )
...
async def add_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not await tg.is_user_authorized():
        await callback.answer(
            "Сначала авторизуй Telegram-аккаунт через /start.",
            show_alert=True,
        )
        return
...
async def target_handler(message: Message, state: FSMContext):
    ...
    if route in routes:
        await state.clear()
        await message.answer(
            "⚠️ Такая пересылка уже существует.",
            reply_markup=menu(),
        )
        return
    ...
    await message.answer(
        f"✅ <b>Пересылка добавлена.</b>\n\n"
        f"Канал: <code>{route['source']}</code>\n"
        f"Чат: <code>{route['target']}</code>",
        reply_markup=menu(),
    )


@dp.callback_query(F.data == "list")
async def list_handler(callback: CallbackQuery):
    if not is_owner(callback.from_user.id):
        await callback.answer("Нет доступа.", show_alert=True)
        return

    await callback.message.edit_text(routes_text(), reply_markup=back())
    await callback.answer()
...
async def new_message_handler(event):
    ...
    for route in matches:
        try:
            ...
            source = await tg.get_entity(route["source"])
